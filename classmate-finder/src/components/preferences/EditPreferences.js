...
const EditPreferences = () => {
  ...
  const [newStudyHabit, setNewStudyHabit] = useState("")
  const [newTimeStudy, setNewTimeStudy] = useState("")
  ...
  const newMajorHandler= (event) => {
    setNewMajor(event.target.value)
  }

  const newPersonalityHandler= (event) => {
    setNewPersonality(event.target.value)
  }

  const newStudyHabitHandler= (event) => {
    setNewStudyHabit(event.target.value)
  }

  const newTimeStudyHandler = (event) => {
    setNewTimeStudy(event.targer.value)
  }

  const changePrefernces = () => {
    const data ={
      newName: newName,
      newYear: newYear,
      newMajor: newMajor,
      newPersonality: newPersonality,
      newStudyHabit: newStudyHabit,
      newTimeStudy: newTimeStudy
    }
    axios.put( "http://localhost:3001/users/editpreferences", data, {
      headers: {
  ...
  return(
    ...
    <form> 
        <h1 userName="preference">Edit Preference</h1>
        <label for="name">Name*:</label>
        <input type="text" required onChange={newNameHandler} placeholder="(First and Last name)"></input>
        ...
        <select name="major" id="major" onChange={newMajorHandler}> 
          <option label=" "></option>
        ...
        <select name="study-habits" id="study-habits" onChange={newStudyHabitHandler} >
          <option label=" "></option>
        ...
        <select 
          name="personality" 
          id="personality" 
          onChange={newPersonalityHandler}
        > 
          <option label=" "> </option>
        ...
        <br></br>
        <br></br> 
        <button onClick={changePrefernces}>Update Preference</button>
